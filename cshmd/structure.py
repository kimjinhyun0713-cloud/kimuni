...
class DATA():
    ...
    def writeCif(self, template=None):
        ...
        if hasattr(self, "outfile"):
            Modifier.outfile = self.outfile
        else:
            while True:
                Modifier.outfile = f"{self.base}.{version:02d}.cif"
                if not os.path.exists(Modifier.outfile):
                    break
                version += 1
                
        with open(Modifier.outfile, "w") as o:
            o.write(template)
            print(f"{Modifier.outfile} was created")
