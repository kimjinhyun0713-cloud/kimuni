...
#        df = df.set_index("Structure")
    ...
    def remove_BT(self, target: int | list, termination: bool, location: str):
        ...
        maska1 = self.data[target_O, 3] < self.model["BT"]["surface"][0][1]
        maska2 = self.data[target_O, 3] > self.model["BT"]["surface"][0][0]
        maskb1 = self.data[target_O, 3] < self.model["BT"]["surface"][1][1]
        maskb2 = self.data[target_O, 3] > self.model["BT"]["surface"][1][0]
        maska = maska1 & maska2
        maskb = maskb1 & maskb2
