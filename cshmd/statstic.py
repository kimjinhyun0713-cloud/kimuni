...
class Ab_stats(PKL):
    ...
    def _setting_(self):
        if isinstance(self.stats[0][0], list):
            self.connected_stats = self.stats[0] # (要修正)csh_analyzer.stdout_statistics
            self.all_stats = self.stats[1] #  (要修正)csh_analyzer.stdout_statistics
        else:
            self.connected_stats = self.stats[1] # (要修正)csh_analyzer.stdout_statistics
    ...
    def label2Catype(self, label):
        ...
        label = sorted(label,
                       key=lambda x: order.get(x[0], float("inf")))
        label = label[0]
        if label[0] == "free":
            return "Ca_free"
        elif label[0] == "CT_O":
    ...
    def label2CO3type(self, label):
        if label == ():
            return "H_free"
        sort_key = ["PT_H", "NT_H", "NT_H_", "bt_H", "bt_H_",
                    "BT_H", "BT_H_"]
        order = {k: i for i, k in enumerate(sort_key)}
        label = sorted(label,
                       key=lambda x: order.get(x[0], float("inf")))
