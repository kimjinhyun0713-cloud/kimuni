...
class LAMMPSTRJ():
    ...
    def to_fract(self):
        for i in range(self.data.shape[0]):
            self.data[i, :, 1:4] = self.data[i, :, 1:4] @ np.linalg.inv(self.matrix[i, :, :].T)
...
class CIF():
    def __init__(self, fname):
        with open(fname) as o:
            ...
            self.lattice = [float(v) for v in re.findall(lattice_ptn, read)]
            self.angle = [float(v) for v in re.findall(angle_ptn, read)]
            ptn_col = r"_atom_site_(.*)"
            self.col = re.findall(ptn_col, read)
            data = read.split(self.col[-1])[-1]
            data = np.array([s.split() for s in data.strip().split("\n")], dtype=object).reshape(-1, len(self.col))
        self.data = pd.DataFrame(data, columns=self.col)
        self.data["fract_x"] = self.data["fract_x"].astype(float)
        self.data["fract_y"] = self.data["fract_y"].astype(float)
        self.data["fract_z"] = self.data["fract_z"].astype(float)
        self.matrix, self.V = setMatrix(self.lattice, self.angle)
        self.xyz = self.data[["fract_x", "fract_y", "fract_z"]]
        self.symbol = self.data["type_symbol"]
        
        
        

    def to_cartesian(self):
        self.data[["fract_x", "fract_y", "fract_z"]] = self.data[["fract_x", "fract_y", "fract_z"]] @ self.matrix.T
        self.xyz = self.data[["fract_x", "fract_y", "fract_z"]]
