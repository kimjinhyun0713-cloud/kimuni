...
from cshmd.util import cif_head
from cshmd.functions import setMatrix
...
#                    raise ValueError("bond")
    ...
    def makeSnapshot(self):
        for n in LAMMPSTRJ.index:
            ...
            self.data[:, 1:4] = self.data[:, 1:4] @ np.linalg.inv(self.matrix).T 
...
def main():
    ...
    lammpstrj -> ciffile    
    """.lstrip()
    par = argparse.ArgumentParser(description=description, prog="make Ciffile")
    par.add_argument('infile', nargs="?",
                     help="入力しなければdirectoryの一番前のlammpstrjをとる")
    par.add_argument('-i', '--index', nargs="+", type=int,
                     help="変換したいindexを指定（e.g. 0 11 200）")
    par.add_argument('-l', '--len_index',  type=int, default=0,
                     help="一定間隔でtrjのsnapshotを変換(最初の100ステップはスキップ)")
    par.add_argument('--clayff', default=False, action="store_true",
                     help="clayffスタイルでlabeling")
    par.add_argument('-z', '--zshift', type=float, default=0,
                     help="C軸に沿って、系全体をargs.shiftだけ上げる")
    par.add_argument('-s', '--symbol', default=False, action="store_true",
                     help="Ciffileのsymbolを外部から '.npy'でinput")
    par.add_argument('-o', '--out', default="cif",
                     help="Ciffileを保存するdirectoryの名前")
    args = par.parse_args()

    infile = args.infile if args.infile else glob.glob("./*.lammpstrj")[0]
    LAMMPSTRJ.setLammpstrj(infile)
    for w in ["index", "len_index", "out"]:
        setattr(LAMMPSTRJ, w, getattr(args, w))
    for w in ["zshift", "clayff"]:
